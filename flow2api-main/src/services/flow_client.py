...
class FlowClient:
    ...
    async def _get_api_captcha_token(self, method: str, project_id: str, action: str = "IMAGE_GENERATION") -> Optional[str]:
        ...
        if method == "yescaptcha":
            ...
            # YesCaptcha 降级顺序：M1S9 -> M1S7 -> M1
            task_types = ["RecaptchaV3TaskProxylessM1S9", "RecaptchaV3TaskProxylessM1S7", "RecaptchaV3TaskProxylessM1"]
        elif method == "capmonster":
            client_key = config.capmonster_api_key
            base_url = config.capmonster_base_url
            task_types = ["RecaptchaV3TaskProxyless"]
        elif method == "ezcaptcha":
            client_key = config.ezcaptcha_api_key
            base_url = config.ezcaptcha_base_url
            task_types = ["ReCaptchaV3TaskProxylessS9"]
        ...
        try:
            ...
            async with AsyncSession() as session:
                ...
                for task_type in task_types:
                    debug_logger.log_info(f"[reCAPTCHA {method}] 尝试使用 task_type: {task_type}")
                    ...
                    if not task_id:
                        ...
                        error_code = result_json.get('errorId', -1)
                        debug_logger.log_warning(f"[reCAPTCHA {method}] task_type={task_type} 创建任务失败 (errorId={error_code}): {error_desc}")
                        last_error = error_desc
                        
                        # 如果还有下一个 task_type 可以尝试，继续循环
                        if task_type != task_types[-1]:
                            await asyncio.sleep(1)
                            continue
                        else:
                            # 所有 task_type 都尝试过了
                            debug_logger.log_error(f"[reCAPTCHA {method}] 所有 task_type 都失败，最后错误: {last_error}")
                            return None
                    ...
                    for i in range(40):
                        ...
                        if status == 'ready':
                            ...
                            if response:
                                debug_logger.log_info(f"[reCAPTCHA {method}] Token获取成功 (task_type={task_type})")
                                return response
                            else:
                                debug_logger.log_warning(f"[reCAPTCHA {method}] 任务完成但缺少 gRecaptchaResponse")
                                break
                        elif status == 'failed':
                            error_desc = result_json.get('errorDescription', 'Task failed')
                            debug_logger.log_warning(f"[reCAPTCHA {method}] 任务失败: {error_desc}")
                            last_error = error_desc
                            # 如果还有下一个 task_type 可以尝试，跳出轮询循环
                            if task_type != task_types[-1]:
                                await asyncio.sleep(1)
                                break
                            else:
                                debug_logger.log_error(f"[reCAPTCHA {method}] 所有 task_type 都失败")
                                return None

                        await asyncio.sleep(3)
                    else:
                        # 轮询超时
                        debug_logger.log_warning(f"[reCAPTCHA {method}] task_type={task_type} 轮询超时")
                        last_error = "Timeout waiting for token"
                        if task_type != task_types[-1]:
                            await asyncio.sleep(1)
                            continue
                        else:
